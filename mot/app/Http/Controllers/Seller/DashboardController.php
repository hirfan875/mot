...
class DashboardController extends Controller
{
    ...
    // show dashboard page
    public function index(FilterProductsService $filterProductsService,Request $request)
    {
        ...
        $query = StoreOrder::with('order.customer')->where('store_id', $user->store_id)->whereHas('order',  function($query){ $query->whereNotIn('status', [Order::UNIITIATED_ID, Order::CONFIRMED_ID, Order::CANCELLED_ID, Order::DELIVERY_FAILURE_ID]);});
        ...
        $query1 = StoreOrder::with('seller')->where('store_id', $user->store_id)->whereHas('order',  function($query){ $query->whereNotIn('status', [Order::UNIITIATED_ID, Order::CONFIRMED_ID, Order::CANCELLED_ID, Order::DELIVERY_FAILURE_ID]);});
        ...
        $query2->whereHas('store_order', function ($query2) use ($user) {
            return $query2->where('store_id', $user->store_id);
        });
        ...
        $total_active_products = $filterProductsService->byStore($user->store_id)->setActiveFilter()->count();
        $total_products = totalProductsByStore($user->store_id);
        

        $total_orders = StoreOrder::whereNotIn('status', [StoreOrder::UNIITIATED_ID, StoreOrder::CONFIRMED_ID, StoreOrder::CANCELLED_ID, StoreOrder::DELIVERY_FAILURE_ID, StoreOrder::RETURN_ACCEPTED_ID, StoreOrder::TERMINATED_ID])->whereStoreId($user->store_id)->count();
        $total_orders_CANCELLED_ID = StoreOrder::whereStatus(StoreOrder::CANCELLED_ID)->whereStoreId($user->store_id)->count();
        $total_orders_RETURN_ACCEPTED_ID = StoreOrder::whereStatus(StoreOrder::RETURN_ACCEPTED_ID)->whereStoreId($user->store_id)->count();
        $total_orders_Delivered = StoreOrder::whereStatus(StoreOrder::DELIVERED_ID)->whereStoreId($user->store_id)->count();
                $store = Store::where('id' , auth::user()->store->id)->get();
        ...
        return view('seller.dashboard', [
            ...
            'store' =>  $store,
            'category' => $category,
            'total_orders_cancelled' => $total_orders_CANCELLED_ID,
            'total_orders_return' => $total_orders_RETURN_ACCEPTED_ID,
