        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header">
              {{ $title }}
              <a class="btn btn-warning btn-sm pull-right ml-2" href="{{ route('seller.store.questions.view.archived') }}">{{ __('View Archived') }}</a>
            </div>
            <div class="card-body">
              <!-- alerts -->
              ...
              <div class="alert alert-danger">{{ __($message) }}</div>
              @enderror
              <form action="{{ route('seller.store.questions.bulk.actions') }}" method="post">
                @csrf
                ...
                <table class="table table-responsive-sm" id="datatables">
                  <thead>
                    <tr>
                      ...
                      <th class="text-center" width="9%">{{ __('Status') }}</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        <div class="custom-control custom-checkbox">
                          <input type="checkbox" class="custom-control-input" name="questions[]" id="question_{{ $row->id }}" value="{{ $row->id }}">
                          <label class="custom-control-label" for="question_{{ $row->id }}"></label>
                        </div>
                      </td>
                      ...
                      <td class="text-center">
                        <x-admin.detail-button :url="route('seller.store.questions.detail', ['question' => $row->id])" />
                        <a href="{{ route('seller.store.questions.archive', ['question' => $row->id]) }}" class="btn btn-danger btn-sm mb-1" onclick="return window.confirm('{{ __('Are you sure you want to archive Question # :id', ['id' => $row->id.'?']) }}');">{{ __('Archive') }}</a>
                        <x-admin.delete-button :url="route('seller.store.questions.delete', ['question' => $row->id])" :title="__('Question #').$row->id" />
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
