                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="text-center">
                      <a href="{{ route('seller.orders.detail', ['order' => $row->store_order_id]) }}" class="btn btn-outline-primary btn-sm mb-1">{{ __('Detail') }}</a>
                      @if ($row->status == $row::PENDING)
                        @foreach ($row->store_order->getPossibleStatusButtonAdmin() as $statusID => $status)
                          <a href="{{ route('seller.orders.update.status', ['order' => $row->store_order->id, 'status' => $statusID]) }}" class="btn btn-outline-primary btn-sm mb-1">{{ $status }}</a>
                        @endforeach
                      @endif
                    </td>
                  </tr>
