        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header">
              {{ __($title) }}
              <x-admin.add-button :url="route('seller.flash.deals.add')" />
            </div>
            <div class="card-body">
              <!-- alerts -->
                  ...
                  @foreach ($records as $row)
                    @if(isset($row->product->title))
                      <tr>
                        ...
                        <td><a href="{{ route('seller.flash.deals.edit', ['deal' => $row->id]) }}">{{ isset($row->product->product_translates) ? $row->product->product_translates->title : $row->product->title }}</a></td>
                        ...
                        <td class="text-center">
                          <x-admin.edit-button :url="route('seller.flash.deals.edit', ['deal' => $row->id])" />
                          <x-admin.delete-button :url="route('seller.flash.deals.delete', ['deal' => $row->id])" :title="$row->title" />
                          @if (!$row->is_approved)
...
@push('footer')
  ...
  <!-- update status -->
  <x-admin.status-update-js :url="route('seller.flash.deals.update.status')" />
@endpush
