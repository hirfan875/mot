...
<ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="{{ route('seller.products') }}">{{ $section_title }}</a></li>
    <li class="breadcrumb-item active">{{ $title }}</li>
</ol>
<div class="container-fluid">
    <div class="animated fadeIn">
        <form action="{{ route('seller.products.edit', ['product' => $row->id]) }}" method="POST" enctype="multipart/form-data" id="edit_form">
            @csrf
            ...
            <div class="row">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-9">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title[{{$val->id}}]" id="title{{$val->id}}" class="form-control" value="{{ old('title[$val->id]', $ctitle) }}" {{$row->is_default == 'Yes' ? 'required' : ''}} {{$val->is_default == 'Yes' ? 'required' : ''}} title="{{__('Please fill out this field')}}">
                                        @error('title[{{$val->id}}]')
                                ...
                                <div class="col-sm-9">
                                    <div class="form-group">
                                        <label for="slug">{{ __('slug') }}<sup style="color:red;">*</sup></label>
                                ...
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        ...
                                        <select name="type" id="type" class="custom-select" onchange="selectProductType(this.value)" disabled>
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="stock" id="stock" class="form-control number" value="{{ old('stock', $row->stock) }}" maxlength="4" onfocusout="updateVariations(this.value, 'stock')" @if ($type === 'variable') disabled @endif>
                                    </div>
                                </div>
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label for="price">{{ __('Price') }}<sup style="color:red;">*</sup></label>
                                        <input type="text" name="price" id="price" class="form-control number" value="{{ old('price', $row->price) }}" onfocusout="updateVariations(this.value, 'price')" >
                                        @error('price')
                                        <span class="invalid-feedback d-block" role="alert"> <strong>{{ __($message) }}</strong> </span>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-sm-3 dimensions" style="display: {{ $type === 'variable' ? 'none' : 'block' }}">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="weight" id="weight" class="form-control number" value="{{ old('weight', $row->weight) }}">
                                    </div>
                                </div>
                                <div class="col-sm-3 dimensions" style="display: {{ $type === 'variable' ? 'none' : 'block' }}">
                                    <div class="form-group">
                                        <label for="length">{{ __('Length') }}</label>
                                        <input type="text" name="length" id="length" class="form-control number" value="{{ old('length', $row->length) }}">
                                    </div>
                                </div>
                                <div class="col-sm-3 dimensions" style="display: {{ $type === 'variable' ? 'none' : 'block' }}">
                                    <div class="form-group">
                                        <label for="height">{{ __('Height') }}</label>
                                        <input type="text" name="height" id="height" class="form-control number" value="{{ old('height', $row->height) }}" >
                                    </div>
                                </div>
                                <div class="col-sm-3 dimensions" style="display: {{ $type === 'variable' ? 'none' : 'block' }}">
                                    <div class="form-group">
                                        <label for="width">{{ __('Width') }}</label>
                                        <input type="text" name="width" id="width" class="form-control number" value="{{ old('width', $row->width) }}" >
                                    </div>
                                </div>
                                <div class="col-sm-4">
                                ...
                                <div class="col-sm-12" id="advance"  style="display: none;" >
                                    <div class='row'>
                                        ...
                                        <div class="col-sm-4"  id="mot_fee_box" >
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="mot_fee" id="mot_fee" class="form-control number" value="{{ $mot_commission_amount }}" disabled>
                                            </div>
                                        </div>
                                        @endcan
                                        <div class="col-sm-4"  id="discount_box" style="display: {{ $type === 'variable' ? 'none' : 'block' }}">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="discount" id="discount" class="form-control number" value="{{ old('discount', $row->discount) }}" onfocusout="updateVariations(this.value, 'discount')">
                                                @error('discount')
                                                <span class="invalid-feedback d-block" role="alert"> <strong>{{ __($message) }}</strong> </span>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-sm-4 dimensions" style="display: {{ $type === 'variable' ? 'none' : 'block' }}">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="volume" id="volume" class="form-control number" value="{{ old('volume', $row->volume) }}" >
                                            </div>
                                        </div>
                                    </div>
                                    @php
                                    ...
                                    <div class="form-group">
                                        <textarea name="short_description[{{$val->id}}]" id="short_description_{{$val->id}}" cols="30" rows="5" class="form-control TinyEditor">{!! old('short_description[{{$val->id}}]', $cShortDescription) !!}</textarea>
                                    </div>
                                    @endforeach
                                    <h3>{{ __('Additional Information') }}</h3>
                                    <div class="form-group">
                                        <textarea name="additional_information" id="additional_information" cols="30" rows="5" class="form-control TinyEditor">{!! old('additional_information', $row->additional_information) !!}</textarea>
                                    </div>
                                </div>
                            </div>
                            <!-- file upload -->
                            <x-admin.file-upload :name="'image'" :file="$row->image" :thumbnail="$row->getMedia('image', 'thumbnail')" :croproute="'seller'" :croptype="'product_main'" :imageid="$row->id" />
                            @php
                            ...
                            <div class="form-group" id="bundle_products_box" style="display: {{ $type === 'bundle' ? 'block' : 'none' }}">
                                ...
                                <select name="bundle_products[]" id="bundle_products" class="select2-custom-select select2-multiple" multiple>
                            ...
                            <div id="variable_box" style="display: {{ $type === 'variable' ? 'block' : 'none' }}">
                                ...
                                <div id="load_attributes">
                                    ...
                                    <div class="form-group" id="attribute_{{ $r->id }}">
                                        ...
                                        <select name="attribute_options_{{ $r->id }}[]" id="attribute_options_{{ $r->id }}" class="custom-select select2-multiple" multiple onchange="selectAttributeOptions()">
                                            @foreach ($r->options as $option)
                                            <option value="{{ $option->id }}" data-title="{{ $option->title }}" @if( in_array($option->id, $product_attributes_optoins_ids) ) selected @endif >{{ $option->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @endforeach
                                            ...
                                            <tbody id="load_variations">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center align-middle">
                                                        {{ $r->option->title }}
                                                        <input type="hidden" name="variations[{{ $key }}][options][]" value="{{ $r->option_id }}" />
                                                        <input type="hidden" name="variations[{{ $key }}][attributes][]" value="{{ $r->attribute_id }}" />
                                                    </td>
                                                    @endforeach
                                                    <td class="text-center">
                                                        <input type="hidden" name="variations[{{ $key }}][id]" value="{{ $variation->id }}" />
                                                        <input type="text" name="variations[{{ $key }}][sku]" class="form-control" value="{{ $variation->store_sku }}" required title="{{__('Please fill out this field')}}">
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][price]" class="form-control variation-price number" value="{{ $variation->price }}" required title="{{__('Please fill out this field')}}">
                                                    </td>
                                                    <td class="text-center">
                                                    ...
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][stock]" class="form-control variation-stock number" value="{{ $variation->stock }}" maxlength="4" required title="{{__('Please fill out this field')}}">
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][mot_fee]" class="form-control variation-mot_fee number" value="{{ $variation->mot_fee }}">
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][weight]" class="form-control variation-weight number" value="{{ $variation->weight }}">
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][length]" class="form-control variation-length number" value="{{ $variation->length }}">
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][height]" class="form-control variation-height number" value="{{ $variation->height }}">
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][width]" class="form-control variation-width number" value="{{ $variation->width }}">
                                                    </td>
                                                    <td class="text-center">
                                                        <input type="text" name="variations[{{ $key }}][volume]" class="form-control variation-volume number" value="{{ $variation->volume }}">
                                                    </td>
                                                    <td class="text-center">
                                                        @php
                                                        $field_name = "variations[$key][image]";
                                                        @endphp
                                            <x-admin.file-upload :name="$field_name" :file="$variation->image" :thumbnail="$variation->getMedia('image', 'thumbnail')" :imageid="$variation->id" />
                                            </td>
                                            </tr>
                                            @endforeach
                                            </tbody>
                            ...
                            <div id="template-preview" style="display: none">
                                <div class="dz-preview dz-file-preview">
                                    ...
                                    <a class="dz-remove dz-crop-link" href="javascript:;" target="_blank">{{ __('Crop file') }}</a>
                                </div>
                            </div>
                ...
                <div class="col-lg-3">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div id="errorTxt">
                                @error('categories') <label class="error" style="display: block;">{{ __($message) }}</label> @enderror
                            </div>
                            @php
                            ...
                            <div id="categories_box" style="height: 400px; overflow: auto">
                                ...
                                <!--                  @foreach ($cat->subcategories as $sub)
                                                  <div class="custom-control custom-checkbox ml-4">
                                                    <input type="checkbox" class="custom-control-input" name="categories[]" id="cat_{{ $sub->id }}" value="{{ $sub->id }}" required @if ( in_array($sub->id, $selected_categories) ) checked @endif onchange="calculateMotFee()">
                                                    <label class="custom-control-label" for="cat_{{ $sub->id }}">{{ $sub->title }}</label>
                                                  </div>
                                                  @endforeach-->
                                @endif
                                @endforeach
  ...
  <script type="text/javascript" charset="utf-8">
    ...
    var myDropzone = new Dropzone("#dropzone", {
      ...
      init: function() {
        ...
        gallery.map(function(r, index){
          ...
          $(".dz-preview").eq(index).find('.dz-crop-link').prop('href', '{{ route('seller.media.crop') }}?type=product&foreign_id='+product_id+'&image_id='+r.id);
        });
    ...
    myDropzone.on("success", function(file, serverFileName) {
      ...
      if ( serverFileName.gallery ) {
        ...
        if ( get_id.id ) {
          ...
          $(file.previewElement).find('.dz-crop-link').prop('href', '{{ route('seller.media.crop') }}?type=product&foreign_id='+product_id+'&image_id='+get_id.id);
        }
      }
    });
    ...
    $("#dropzone").sortable({
      ...
      update: function (e, ui) {
        ...
        // send post request
        axios.post('{{ route('seller.products.gallery.update.order') }}', {
          items: sortedMedia,
          id: {{ $row->id }}
        }).then(function (response) {
          $('button#SaveBtn').html( $('button#SaveBtn').data('original') ).prop('disabled', false);
