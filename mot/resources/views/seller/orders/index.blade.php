              ...
              <form action="{{ route('seller.orders') }}" method="GET">
                <div class="form-row align-items-center mb-3">
                  ...
                  <div class="col-auto my-1">
                    ...
                    <a href="{{ route('seller.orders') }}" class="text-link" style="text-decoration: underline">{{__('Reset Filters')}}</a>
                    @endif
                  </div>
                </div>
              </form>
              <table class="table table-responsive-sm" id="datatables">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td class="text-left">{{ $row->order->customer->name }} {{ ($row->order->customer->is_guest == 1 ) ? 'Guest':''}}</td>
                    ...
                    <td class="text-center">
                      <a href="{{ route('seller.orders.detail', ['order' => $row->id]) }}" class="btn btn-outline-primary btn-sm mb-1"><i class="fa fa-eye"></i></a>
                      {{-- <a class="btn btn-warning btn-sm pull-right ml-2" href="{{ route('admin.orders.detail.export', ['storeOrder' => $row->id]) }}"><i class="fa fa-download"></i></a> --}}
                    </td>
                  </tr>
                  @endforeach
                </tbody>
