        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header">
              {{ __($title) }}
              <x-admin.add-button :url="route('seller.staff.add')" />
            </div>
            <div class="card-body">
              <!-- alerts -->
              ...
              <table class="table table-responsive-sm" id="datatables">
                <thead>
                  <tr>
                    ...
                    <th class="text-center" width="15%">{{ __('Phone') }}</th>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><a href="{{ route('seller.staff.edit', ['staff' => $row->id]) }}">{{ $row->name }}</a></td>
                    <td>{{ $row->email }}</td>
                    <td class="text-center">{{ $row->phone }}</td>
                    <td class="text-center">{{ isset($row->updated_at) ? date_format($row->updated_at,"d/m/Y H:i:s") : '' }}</td>
                    <td class="text-center">
                      <x-admin.edit-button :url="route('seller.staff.edit', ['staff' => $row->id])" />
                      <x-admin.delete-button :url="route('seller.staff.delete', ['staff' => $row->id])" :title="$row->name" />
                    </td>
                  </tr>
                  @endforeach
                </tbody>
...
@push('footer')
  ...
  <!-- update status -->
  <x-admin.status-update-js :url="route('seller.staff.update.status')" />
@endpush
