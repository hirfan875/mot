        ...
        <div class="col-lg-6">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <form action="{{ route('admin.tabbed.products.add') }}" method="POST" id="add_form">
                @csrf
                ...
                <div class="form-group" id="categories_box" style="display: {{ $type === 'category' ? 'block' : 'none' }}">
                  ...
                  <select name="category_id" id="category_id" class="custom-select select2" >
                    ...
                    <option value="{{ $category->id }}">{{$category->category_translates ? $category->category_translates->title : $category->title}}</option>
                    @if ($category->subcategories)
                    @foreach ($category->subcategories as $subcategory)
                    <option value="{{ $subcategory->id }}">&nbsp;&nbsp;&nbsp;&nbsp;{{$subcategory->category_translates ? $subcategory->category_translates->title : $subcategory->title}}</option>
                    @endforeach
                    @endif
                    @endforeach
                  </select>
