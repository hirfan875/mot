        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-header">
              {{ __($title) }}
              <x-admin.add-button :url="route('admin.tabbed.products.add')" />
            </div>
            <div class="card-body">
              <!-- alerts -->
              ...
              <table class="table table-responsive-sm" id="datatables">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><a href="{{ route('admin.tabbed.products.edit', ['item' => $row->id]) }}">{{ __($row->title) }}</a></td>
                    <td class="text-center">{{ __($row->type) }}</td>
                    <td class="text-center">{{ date_format($row->updated_at,"d/m/Y H:i:s") }}</td>
                    <td class="text-center">
                      <x-admin.edit-button :url="route('admin.tabbed.products.edit', ['item' => $row->id])" />
                      <x-admin.delete-button :url="route('admin.tabbed.products.delete', ['item' => $row->id])" :title="$row->title" />
                    </td>
                  </tr>
                  @endforeach
                </tbody>
...
@push('footer')
  ...
  <!-- update status -->
  <x-admin.status-update-js :url="route('admin.tabbed.products.update.status')" />
@endpush
