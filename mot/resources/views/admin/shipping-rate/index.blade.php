                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <x-admin.add-button :url="route('admin.shipping.rates.add', ['country' => $country->id])" />
                        </div>
                        <div class="card-body">
                            ...
                            <table class="table table-responsive-sm" id="datatables">
                                ...
                                <tr>
                                    ...
                                    <th width="25%">{{ __('Weight') }}</th>
                                    <th width="25%">{{ __('Rates') }}</th>
                                ...
                                @foreach ($records as $row)
                                    <tr>
                                        ...
                                        <td><a href="{{ route('admin.shipping.rates.edit', ['country' => $country->id, 'shippingRate' => $row->id]) }}">{{ $row->weight }}</a></td>
                                        ...
                                        <td class="text-center">
                                            <x-admin.edit-button :url="route('admin.shipping.rates.edit', ['country' => $country->id, 'shippingRate' => $row->id])" />
                                            <x-admin.delete-button :url="route('admin.shipping.rates.delete', ['country' => $country->id, 'shippingRate' => $row->id])" :title="$row->title" />
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
...
@push('footer')
        ...
        $(document).ready(function(){
            var table = $('#datatables').DataTable({
                ...
                "aoColumnDefs": [
                    ...
                    { 'bSortable': true, 'aTargets': [ 1 ], 'name': 'weight' },
                    { 'bSortable': true, 'aTargets': [ 2 ], 'name': 'rates' },
    ...
    <!-- update status -->
    <x-admin.status-update-js :url="route('admin.shipping.rates.update.status', ['country' => $country->id])" />
@endpush
