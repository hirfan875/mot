...
<div class="container-fluid">
    <div class="animated fadeIn">
        <form action="{{ route('admin.products.add') }}" method="POST" enctype="multipart/form-data" id="add_form">
            @csrf
            <div class="row">
                <div class="col-lg-9">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                @foreach(getLocaleList() as $key => $row)
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label for="title">{{ __('Title ('.$row->title.')') }}</label>
                                        <input type="text" name="title[{{$row->id}}]" id="title_{{$row->id}}" class="form-control" value="{{ old('title.'.$row->id) }}" {{$row->is_default == 'Yes' ? 'required' : ''}} title="{{__('Please fill out this field')}}" autofocus onfocusout="metaTitle({{$row->id}})" />
                                        @error('title[$key]')
                                ...
                                $brand = old('brand');
                                @endphp
                                <div class="col-sm-4" id="brand_box" style="display: {{ $type === 'bundle' ? 'none' : 'block' }}">
                                    <div class="form-group">
                                ...
                                <div class="col-sm-4" id="mot_fee_box" style="display: {{ $type === 'variable' ? 'none' : 'block' }}">
                                  <div class="form-group">
                                    <label for="mot_fee">{{ __('Mot Fee') }}</label>
                                    <input type="text" name="mot_fee" id="mot_fee" class="form-control number" value="{{ old('mot_fee') }}" disabled>
                                  </div>
                              </div>
                                <div class="col-sm-4">
                                    <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select name="tags[]" id="tags" class="custom-select select2-multiple" multiple>
                                    @foreach ($tags as $r)
                                    <option value="{{ $r->id }}" @if( in_array($r->id, $selected_tags) ) selected @endif >{{ $r->title }}</option>
                                    @endforeach
                                </select>
                                @error('tags')
                                <span class="invalid-feedback d-block" role="alert"> <strong>{{ __($message) }}</strong> </span>
                            ...
                            @endforeach
                            @foreach(getLocaleList() as $row)
                            <h3>{{ __('Description ('.$row->title.')') }}</h3>
                            <div class="form-group">
                                <textarea onblur="metaDesc({{$row->id}})" name="data[{{$row->id}}]" id="data{{$row->id}}" cols="30" rows="5" class="form-control TinyEditor" >{!! old('data[{{$row->id}}]') !!}</textarea>
                            </div>
                            @endforeach
              ...
              <div class="card-body">
                ...
                <div id="categories_box" style="height: 400px; overflow: auto">
                  ...
                  <div class="custom-control custom-checkbox mb-1">
                    <input type="checkbox" class="custom-control-input" name="categories[]" id="cat_{{ $cat->id }}" value="{{ $cat->id }}"  @if ( in_array($cat->id, $selected_categories) ) checked @endif onchange="calculateMotFee()">
                    <label class="custom-control-label" for="cat_{{ $cat->id }}">{{$cat->category_translates ? $cat->category_translates->title : $cat->title}}</label>
                  </div>
  ...
  <script type="text/javascript" charset="utf-8">
    ...
    function appendVariationRow(variation, index) {
      ...
      let template = `<tr>
        ...
        <td class="text-center">
          <input type="text" name="variations[${index}][volume]" class="form-control variation-volume number" value="">
        </td>
        <td class="text-center">
