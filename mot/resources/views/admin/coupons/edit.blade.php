            ...
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.coupons.edit', ['coupon' => $row->id]) }}" method="POST" id="edit_form" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        <label for="coupon_code">{{ __('Title') }}</label>
                                ...
                                <div class="col-sm-2">
                                    <div class="form-group">
                                        ...
                                        <select name="store" id="store" class="form-control select2">
                                            ...
                                            @foreach ($stores as $r)
                                            <option value="{{ $r->id }}" {{$r->id == $row->store_id ? 'selected' : null}}>{{ $r->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <div class="custom-control custom-radio custom-control-inline">
                                            <input type="radio" id="type" name="type" class="custom-control-input" value="fixed" onchange="DiscountType(this.value)" required @if ( $type === 'fixed' ) checked @endif>
                                            <label class="custom-control-label" for="type">{{ __('Fixed Amount') }}</label>
                                        </div>
                                        <div class="custom-control custom-radio custom-control-inline">
                                            <input type="radio" id="type2" name="type" class="custom-control-input" value="percentage" onchange="DiscountType(this.value)" required @if ( $type === 'percentage' ) checked @endif>
                                            <label class="custom-control-label" for="type2">{{ __('Percentage') }}</label>
                                        </div>
                                        <div class="custom-control custom-radio custom-control-inline">
                                            <input type="radio" id="type3" name="type" class="custom-control-input" value="get_free" onchange="DiscountType(this.value)" required @if ( $type === 'get_free' ) checked @endif>
                                            <label class="custom-control-label" for="type3">{{ __('Buy & Get Free') }}</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6" id="discount_box" style="display: {{ $type != 'get_free' ? 'block' : 'none' }};">
                                    <div class="form-group">
                                ...
                                <div class="col-sm-6" id="limit_box1" style="display: {{ $type == 'get_free' ? 'block' : 'none' }};">
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label for="buy_limit">{{ __('Buy Limit') }}</label>
                                                <input type="text" name="buy_limit" id="buy_limit" class="form-control number" value="{{ old('buy_limit', $row->buy_limit) }}" autocomplete="off" >
                                                @error('buy_limit')
                                                <span class="invalid-feedback d-block" role="alert"> <strong>{{ __($message) }}</strong> </span>
                                                @enderror
                                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="limit" id="limit" class="form-control number" value="{{ old('limit', $row->total_limit) }}" autocomplete="off" required>
                                                @error('limit')
                                                <span class="invalid-feedback d-block" role="alert"> <strong>{{ __($message) }}</strong> </span>
                                                @enderror
                                            </div>
                            ...
                            <div id="choose_products_box" style="display: {{ $applies_to == 2 ? 'block' : 'none' }};">
                                <div class="form-group" style="position: relative;">
                                    ...
                                    <select name="products[]" id="products" class="select2-ajax-products" multiple>
                                        ...
                                        <option value="{{ $r->id }}" selected>{{ $r->title }} - {{$r->sku}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
<!--                                        <div class="col-sm-6">
                                            <label for="sub_total">{{ __('Maximun Subtotal Limit') }}</label>
                                            <input type="text" name="sub_total_max" id="sub_total_max" class="form-control number" value="{{ old('sub_total_max', $row->sub_total_max) }}">
                                            @error('sub_total_max')
                                            <span class="invalid-feedback d-block" role="alert"> <strong>{{ __($message) }}</strong> </span>
                                            @enderror
                                        </div>-->
...
@push('footer')
  ...
  <script type="text/javascript" charset="utf-8">
    ...
    function UsageLimit1(type) {
      if ( type == 2 ) $('#limit_box1').show(); else $('#limit_box1').hide();
    }
