                    ...
                    <div class="card-header">
                        ...
                        <div class="btn-group pull-right mr-2">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a href="{{ route('admin.reports.group.sales.stores.export', ['type' => 'xlsx']) }}?startDate={{$startDate}}&endDate={{$endDate}}&status={{$status}}&groupby={{$groupby}}&store={{$store_id}}" class="dropdown-item">Excel</a>
                                <a href="{{ route('admin.reports.group.sales.stores.export', ['type' => 'csv']) }}?startDate={{$startDate}}&endDate={{$endDate}}&status={{$status}}&groupby={{$groupby}}&store={{$store_id}}" class="dropdown-item">CSV</a>
                                <a href="{{ route('admin.reports.group.sales.stores.export', ['type' => 'pdf']) }}?startDate={{$startDate}}&endDate={{$endDate}}&status={{$status}}&groupby={{$groupby}}&store={{$store_id}}" class="dropdown-item">PDF</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <form  action="{{ route('admin.reports.group.sales.stores') }}" method="get" class="mb-3">
                                    ...
                                    <tr>
                                        ...
                                        <td width="20%" class="pr-3">
                                            ...
                                            <select name="status" id="status" class="custom-select ml-2 mr-2">
                                                <option value="">--Ordre Status--</option>
                                        ...
                                        <td width="20%" class="pr-3">
                                            {{__('Select Store')}} 
                                            <select name="store" id="store" class="custom-select ml-2 mr-2">
                        ...
                        <table class="table table-responsive-sm" id="datatables">
                            <thead>
                                <tr>
                                    <th class="text-center" width="10%">{{__('Seller')}}</th>
                                    <th class="text-center" width="10%">{{__('Total Order')}}</th>
                                    <th class="text-center" width="20%">{{__('SubTotal Sales')}}</th>
                                    <th class="text-center" width="20%">{{__('Delivery Fee')}}</th>
                                    <th class="text-center" width="20%">{{__('Total Sales')}}</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">{{__('TRY')}} {{ number_format($row->amountTotal, 2, ".", ","); }}</td>
                                    <td class="text-center">{{__('TRY')}} {{ number_format($row->deliveryFee, 2, ".", ","); }}</td> 
                                    <td class="text-center">{{__('TRY')}} {{ number_format($row->amountTotal + $row->deliveryFee, 2, ".", ","); }}</td> 
...
$(document).ready(function () {
  var table = $('#datatables').DataTable({
      ...
      "autoWidth": false,
      "aoColumnDefs": [
          {'bSortable': true, 'aTargets': [0], 'name': 'seller'},
          {'bSortable': true, 'aTargets': [1], 'name': 'countTotal'},
          {'bSortable': true, 'aTargets': [2], 'name': 'amountTotal'},
          {'bSortable': true, 'aTargets': [3], 'name': 'deliveryFee'},
          {'bSortable': true, 'aTargets': [4], 'name': 'total'},
          {'bSortable': true, 'aTargets': [5], 'name': 'date'}
      ]
  });
