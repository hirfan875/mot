        ...
        <div class="row">
            <div class="col-lg-12">
                    ...
                    <div class="card-header">
                        ...
                        <div class="btn-group pull-right mr-2">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a href="{{ route('admin.reports.group.sales.customers.export', ['type' => 'xlsx']) }}?startDate={{$startDate}}&endDate={{$endDate}}&status={{$status}}&groupby={{$groupby}}&store={{$store_id}}&product={{$product_id}}&customer={{$customer_id}}&category={{$category_id}}" class="dropdown-item">Excel</a>
                                <a href="{{ route('admin.reports.group.sales.customers.export', ['type' => 'csv']) }}?startDate={{$startDate}}&endDate={{$endDate}}&status={{$status}}&groupby={{$groupby}}&store={{$store_id}}&product={{$product_id}}&customer={{$customer_id}}&category={{$category_id}}" class="dropdown-item">CSV</a>
                                <a href="{{ route('admin.reports.group.sales.customers.export', ['type' => 'pdf']) }}?startDate={{$startDate}}&endDate={{$endDate}}&status={{$status}}&groupby={{$groupby}}&store={{$store_id}}&product={{$product_id}}&customer={{$customer_id}}&category={{$category_id}}" class="dropdown-item">PDF</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.reports.group.sales.customers') }}" method="GET">
                            <div class="form-row align-items-center mb-3">
                                ...
                                <div class="col-sm-2 my-1">
                                    <select class="custom-select select2 mr-sm-2" name="store" id="store">
                                        ...
                                        <option value="{{ $store->id }}" {{ $store->id == $store_id ? "selected":"" }} >{{isset($store->store_profile_translates)? $store->store_profile_translates->name : $store->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-sm-2 my-1">
                                    <select class="custom-select select2 mr-sm-2" name="product" id="product">
                                        ...
                                        <option value="{{ $product->id }}" {{ $product->id == $product_id ? "selected":"" }} >{{isset($product->product_translates)? $product->product_translates->title : $product->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-sm-2 my-1">
                                    <select class="custom-select select2 mr-sm-2" name="category" id="category">
                                        <option value="" selected>--{{__('All Category')}}--</option>
                                        @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" {{ $category->id == $category_id ? "selected":"" }} >{{isset($category->category_translates)? $category->category_translates->title : $category->title}}</option>
                                        @endforeach
                                    </select>
                                </div> 
                        ...
                        <table class="table table-responsive-sm" id="datatables">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left" width="20%">{{ __('Customer') }}</th>
                                    <th class="text-left" width="20%">{{ __('Email') }}</th>
                                    <th class="text-center" width="10%">{{ __('Base Order Total') }}</th>
                                    <th class="text-center" width="10%">{{ __('Order Total') }}</th>
                                    <th class="text-center" width="10%">{{ __('Order Date') }}</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-left">{{__('TRY')}} {{ number_format($row->total, 2, ".", ","); }}</td>
                ...
                <!--                    <td class="text-center">
                                      <a href="{{ route('admin.orders.detail', ['order' => $row->id]) }}" class="btn btn-outline-primary btn-sm mb-1">{{ __('Detail') }}</a>
                                    </td>-->
                                </tr>
                                @endforeach
                            </tbody>
