...
@section('content')
  <div class="spinner_box" id="spinner_box" style="display: none">
    <div class="spinner">
      <div class="rect1"></div>
      <div class="rect2"></div>
      <div class="rect3"></div>
              ...
              <table class="table table-responsive-sm" id="datatables">
                ...
                <tbody>
                  ...
                  <td class="text-center">{{ (isset($row->order_number))? $row->order_number:"" }}</td>
                    ...
                    <td class="text-left">{{ $row->customer->name }} - {{ ($row->customer->is_guest == 1 ) ? 'Guest':''}} {{ ($row->order_from != null ) ? $row->order_from :''}}</td>
                    <td class="text-center">{{__($row->currency->code)}}&nbsp;{{convertTryForexRate($row->total, $row->forex_rate, $row->base_forex_rate, $row->currency->code)}} 
                        <br/>{{__('TRY')}} {{ number_format($row->total, 2, ".", ","); }}
                    </td>
                    <td class="text-center">{{ date_format($row->created_at,"d/m/Y H:i:s") }}</td>
                    <td class="text-center">{{ __($row->getStatus($row->status)) }}</td>
                    <td class="text-center">
                      <a href="{{ route('admin.orders.detail', ['order' => $row->id]) }}" class="btn btn-outline-primary btn-sm mb-1"><i class="fa fa-eye"></i></a>
                      ...
                      <a class="btn btn-warning btn-sm pull-right ml-2" href="{{ route('admin.orders.detail.export', ['order' => $row->id]) }}"><i class="fa fa-download"></i></a>
                    </td>
                  </tr>
                  @endforeach
                </tbody>
