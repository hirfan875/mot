...
<ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="{{ route('admin.orders') }}">{{ __($section_title) }}</a></li>
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header">
                        {{ __('Items') }}
                        <x-admin.back-button :url="route('admin.orders')" />
                    </div>
                    <div class="card-body">
                        <!-- alerts -->
                                ...
                                <tbody>
                                    ...
                                    @if($order->getDiscount())
                                        <tr>
                                            ...
                                            <td width="15%" class="text-right">{{__($order->currency->code)}}&nbsp;-{{convertTryForexRate($order->getDiscount(), $order->forex_rate, $order->base_forex_rate, $order->currency->code)}}</td>
                                        </tr>
                                    @endif
                                    <tr>
                                        <th width="85%" class="text-right" colspan="3">{{__('Delivery Fee')}}:</th>
                                    ...
                                    <tr>
                                        <td> {{__('Ordered Currency forex rate')}}: {{$order->currency->code}} : {{ $order->forex_rate }}</td><td colspan="3"> {{__('Ordered Default Currency forex rate')}}: {{ $order->base_forex_rate}}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="col-lg-4">
                <div class="card">
                    <div class="card-header">
                        {{ $title }}
                        <a class="btn btn-warning btn-sm pull-right ml-2" href="{{ route('admin.orders.detail.export', ['order' => $order->id]) }}"><i class="fa fa-download"></i> Invoice</a>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-sm table-borderless">
                                ...
                                @if($order->getDiscount() > 0)
                                    <tr>
                                        <td>{{__('Discount')}}:</td>
                                        <td>{{__($order->currency->code)}}&nbsp;-{{convertTryForexRate($order->getDiscount(), $order->forex_rate, $order->base_forex_rate, $order->currency->code)}}</td>
                                    </tr>
                                @endif
...
<!--                                <tr>
                                    ...
                                    <td>
                                        @foreach ($status_buttons as $statusID => $status)
                                            @if($status == 'Shipped')
                                                @if(!$order->pickup_reponse)
                                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal">{{ __('Print Lable') }}</button>
                                                @endif
                                                @if($order->shipment_reponse)
                                                    <a href="{{asset($order->shipment_reponse->file)}}" target="_blank" class="btn btn-success">{{ __('View Shipment Lable') }}</a>
                                                @endif
                                            @endif
                                        @endforeach
                                    </td>
...
<!--                                    <tr>
                                        ...
                                        <td>
                                            @if($order->pickup_reponse)
                                                {{__('Dispatch Confirmation Number')}}: {{$order->pickup_reponse->dispatch_confirmation}}
                                            @else
...
<div class="modal" id="myModal">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            @php
            $insured_value = isset($order->shipment_requests->insured_value) ? $order->shipment_requests->insured_value : '0';
            $weight = isset($order->shipment_requests->weight) ? $order->shipment_requests->weight : '0.2';
            $length = isset($order->shipment_requests->length) ? $order->shipment_requests->length : '0';
            $width = isset($order->shipment_requests->width) ? $order->shipment_requests->width : '0';
            $height = isset($order->shipment_requests->height) ? $order->shipment_requests->height : '0';
            $shiptimestamp = isset($order->shipment_requests->shiptimestamp) ? $order->shipment_requests->shiptimestamp : date('Y-m-d', strtotime('+1 week'));
            $customer_references = isset($order->shipment_requests->customer_references) ? $order->shipment_requests->customer_references : 'TEST TR-KW';
            @endphp
            <!-- Modal body -->
            <div class="modal-body">
                <form action="{{ route('admin.shipment.request', ['storeOrder' => $order->id]) }}" method="POST" id="add_form">
                    @csrf
...
<div class="modal" id="myModal1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <!-- Modal body -->
            <div class="modal-body">
                <form action="{{ route('admin.pickup.request', ['storeOrder' => $order->id]) }}" method="POST" id="add_form">
                    @csrf
